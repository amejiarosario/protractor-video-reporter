...
VideoReporter.prototype._stopScreencast = function(removeVideo) {
  ...
  if (removeVideo) {
    ...
    console.log('saving into video ' + self._currrentVideoName);
    // console.log('Spec video is in: ' + self._videoPath);
    Fs.renameSync(self._videoPath, self._currrentVideoName);
  }
...
VideoReporter.prototype.specDone = function(result) {
  ...
  if (!self.options.singleVideo) {
    self._currrentVideoName = Path.join(self.options.baseDirectory, result.status.toUpperCase() + ' - ' + result.fullName + '.mov');
